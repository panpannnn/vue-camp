...
function propagate(subs) {
  ...
  queuedEffect.forEach((effect3) => effect3.notify());
}
...
function refreshComputed(computed3) {
  ...
  const prevSub = activeSub2;
  const prevShouldTrack = shouldTrack;
  activeSub2 = computed3;
  shouldTrack = true;
  try {
    prepareDeps(computed3);
    const value = computed3.fn(computed3._value);
    if (dep.version === 0 || hasChanged2(value, computed3._value)) {
      computed3.flags |= 128;
...
var Dep = class {
  ...
  track(debugInfo) {
    if (!activeSub2 || !shouldTrack || activeSub2 === this.computed) {
      return;
    }
    let link2 = this.activeLink;
    if (link2 === void 0 || link2.sub !== activeSub2) {
      link2 = this.activeLink = new Link2(activeSub2, this);
      if (!activeSub2.deps) {
        activeSub2.deps = activeSub2.depsTail = link2;
      } else {
        link2.prevDep = activeSub2.depsTail;
        activeSub2.depsTail.nextDep = link2;
        activeSub2.depsTail = link2;
      }
      addSub(link2);
      ...
      if (link2.nextDep) {
        ...
        activeSub2.depsTail.nextDep = link2;
        activeSub2.depsTail = link2;
        if (activeSub2.deps === link2) {
          activeSub2.deps = next;
        }
      }
    }
    if (activeSub2.onTrack) {
      activeSub2.onTrack(
        extend(
          {
            effect: activeSub2
...
function watch(source, cb, options = EMPTY_OBJ) {
  ...
  effect3 = new ReactiveEffect2(getter);
...
function reactive2(target) {
  return createReactiveObject2(target);
}
var reactiveMap2 = /* @__PURE__ */ new WeakMap();
var reactiveSet = /* @__PURE__ */ new WeakSet();
function createReactiveObject2(target) {
  if (!isObject(target)) {
    return target;
  }
  if (isReactive2(target)) {
    return target;
  }
  const existingProxy = reactiveMap2.get(target);
  if (existingProxy) {
    return existingProxy;
  }
  const proxy = new Proxy(target, mutableHandlers2);
  reactiveMap2.set(target, proxy);
  reactiveSet.add(proxy);
  return proxy;
}
function isReactive2(target) {
...
var RefImpl = class {
  ...
  subsTail;
  constructor(value) {
    this._value = isObject(value) ? reactive2(value) : value;
  }
  get value() {
    if (activeSub) {
      trackRef(this);
  ...
  set value(newValue) {
    if (hasChanged(newValue, this._value)) {
      this._value = isObject(newValue) ? reactive2(newValue) : newValue;
      triggerRef2(this);
    }
  }
};
